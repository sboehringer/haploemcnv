...
#' 
#' @param gene1 A list. Diplotype list of the starting gene, can be obtained via \cofull_regressionde{\link{all_options}}.
...
#' @param full_regression A logical scalar. Whether or not the regression models should contain all haplotype predictors next to the allelic predictors. Only functional when \code{regression} is not \code{NULL}. \lars{Dit is nu waarschijnlijk al overbodig.}
...
grouping_order_all = function(gene1, ..., other_genes = NULL, gene_names = NULL, orderings = NULL, comb_vals = NULL, naive = TRUE, fixed_order = FALSE, candidate_list = NULL, 
  ...
  for(i in 1:nr_other_genes){
    GO_i = grouping_order_iter(gene1 = gene1, other_genes, comb_gene1 = FALSE, reference_info = reference_info, combine_haplos = combine_haplos, regression = NULL,
                               ...
                               comb_vals = comb_vals, CO_perc = CO_perc, CO_thresh = CO_thresh, CO_min = CO_min)
